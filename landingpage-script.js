    ...
    const logginFunction = async (userName, password) => {
      try {
        const response = await fetch(
          `https://localhost:7009/api/Account/GetAccountByLogin/${userName}/${password}`,
          {
            method: "GET",
          }
        );
        ...
        window.location.href = "http://127.0.0.1:5500/appointment-booking.html"; // Redirect to the appointment booking page after successful login
        return data;
      } catch (error) {
...
document.getElementById('registerForm').addEventListener('submit', function(e) {
    ...
    // Hidden fields (already set in HTML)
    const role = 0;
    const isActive = true;
    ...
    // Clear previous errors
    ['username', 'fullName', 'registerEmail', 'dateOfBirth', 'registerPassword', 'confirmPassword'].forEach(clearError);

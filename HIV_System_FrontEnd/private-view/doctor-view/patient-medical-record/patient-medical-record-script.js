...
function renderPatients(patients) {
    ...
    document.querySelectorAll('.view-details-btn').forEach(btn => {
        btn.onclick = async function() {
            ...
            try {
                const response = await fetch(`https://localhost:7009/api/PatientMedicalRecord/GetPatientMedicalRecordById/${record.pmrId}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
                ...
                let html = `<h2>Patient Medical Record</h2>`;
                ...
                // Fetch and display test result if pmrId exists
                if (record.pmrId) {
                    try {
                        const testResultRes = await fetch(`https://localhost:7009/api/TestResult/GetById/${record.pmrId}`, {
                            headers: {
                                'Authorization': `Bearer ${token}`
                            }
                        });
                ...
                document.getElementById('patientDetailContent').innerHTML = `<p style='color:red;'>Error loading patient medical record.</p>`;
                document.getElementById('patientDetailModal').style.display = 'block';
            }
        };
    });

...
const appointmentData = { date: null, time: null, doctor: null, type: null, notes: null };
let currentStep = 1;
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (doctorId) {
        ...
        const token = localStorage.getItem("token");
        const response = await fetch(`https://localhost:7009/api/Doctor/GetDoctorById?id=${doctorId}`, {
            headers: { "Authorization": `Bearer ${token}` }
        });
        if (response.ok) {
    ...
    workSchedule
        ...
        .forEach(ws => {
            ...
            while (current < end) {
                let slotStart = current.toTimeString().substring(0, 5);
                current.setMinutes(current.getMinutes() + 30);
                let slotEnd = current < end ? current.toTimeString().substring(0, 5) : ws.endTime.substring(0, 5);
                slots.push(`${slotStart} - ${slotEnd}`);
            }
        });
    return slots;
